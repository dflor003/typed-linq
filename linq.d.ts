declare module linqjs {
  interface EnumerableStatic {
    ...
    constructor<T>(getEnumerator: () => IEnumerator<T>): IEnumerable<T>;

    choice<T>(...params: T[]): IEnumerable<T>;
    choice<T>(params: T[] | ArrayLike<T>): IEnumerable<T>;

    cycle<T>(...params: T[]): IEnumerable<T>;
    cycle<T>(params: T[] | ArrayLike<T>): IEnumerable<T>;
    ...
    from<T>(obj: T[] | ArrayLike<T>): IEnumerable<T>;
    from<T>(obj: { [name: string]: T; }): IEnumerable<KeyValuePair<string, T>>;
    from(obj: Object): IEnumerable<PropertyValue>;

    make<T>(element: T): IEnumerable<T>;

    matches(input: string, pattern: RegExp): IEnumerable<string>;
    matches(input: string, pattern: string, flags?: string): IEnumerable<string>;
    ...
    repeatWithFinalize<T>(initializer: () => T, finalizer: Transform<T, void>): IEnumerable<T>;
  ...
  let Enumerable: EnumerableStatic;
}

export = linqjs.Enumerable;
